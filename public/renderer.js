...
async function loadModel(filePath, options = {}) {
  ...
  try {
    ...
    const encodedFilePath = await window.electron.getFileUrl(filePath);
    if (!encodedFilePath) throw new Error('Could not get file URL');

    // Extract extension (simple logic)
    let fileExtension = 'stl';
    if (filePath.toLowerCase().endsWith('.3mf') || filePath.toLowerCase().includes('.3mf::')) fileExtension = '3mf';
