...
async function testAIConfig(apiKey, baseURL, model, service = 'openai') {
  ...
  // Make sure we're using a PNG image for testing
  const logoPath = process.platform === 'win32' 
    ? './resources/app.asar/logo.png' 
    : './logo.png';
  ...
  try {
    ...
    const data = await fs.readFile(logoPath, { encoding: 'base64' });
    
    // Log the image format for debugging
    console.log(`Testing AI with image: ${logoPath}`);
    
    // Generate tags using the logo image
    const tags = await generateTagsForImage(data, model);
