...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function addTagToModel(tagName, containerId) {
    ...
    console.log(`Saving tags for container ${containerId}:`, currentTags);
...
async function initializeApp() {
  try {
    ...
    // Check for models without thumbnails and prompt user
    try {
      const modelsWithoutThumbs = await window.electron.getModelsWithoutThumbnails();
      if (modelsWithoutThumbs && modelsWithoutThumbs.length > 0) {
        ...
        if (userChoice === 'Yes') {
          console.log('User chose to generate thumbnails for', modelsWithoutThumbs.length, 'models');
          totalThumbnailsToGenerate = modelsWithoutThumbs.length;
          generatedThumbnailsCount = 0;
          ...
          if (progressDialog && progressBar && progressText) {
            progressDialog.showModal();
            ...
            await window.electron.showMessage('Error', 'Failed to generate thumbnails: Progress UI elements not found');
          }
        }
      }
    } catch (error) {
      console.error('Error checking for models without thumbnails:', error);
    }
...
async function autoSaveMultipleModels(field, value) {
  try {
    ...
    // Remove transaction calls that are causing errors
    ...
    for (const filePath of selectedModels) {
      ...
      if (model) {
        ...
        if (field === 'tags') {
          ...
          model.tags = Array.isArray(value) ? value : [];
        } else {
          model[field] = value;
        }
    ...
    console.log(`Updated ${field} to ${JSON.stringify(value)} for ${selectedModels.size} models`);
    return true;
  } catch (error) {
