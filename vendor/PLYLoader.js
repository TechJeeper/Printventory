( function () {
	...
	/**
	 * THREE loader for PLY (Stanford Triangle Format).
	 * Based on three.js r128 examples/js/loaders/PLYLoader.js.
	 * Header length uses UTF-8 bytes so it works in Web Workers (no Blob).
	 */
	...
	class PLYLoader extends THREE.Loader {
		...
		parse( data ) {
			...
			function parseHeader( text ) {
				...
				let headerText = '';
				let headerLength = 0;
				const result = patternHeader.exec( text );
			...
			function parseASCII( text, header ) {
				...
				let body = '';
				const result = patternBody.exec( text );
				if ( result !== null ) body = result[ 1 ];
			...
			function parseBinary( arrayBuffer, header ) {
				...
				for ( let currentElement = 0; currentElement < header.elements.length; currentElement ++ ) {
					...
					for ( let currentElementCount = 0; currentElementCount < header.elements[ currentElement ].count; currentElementCount ++ ) {
						...
						const result = binaryReadElement( body, loc, header.elements[ currentElement ].properties, little_endian );
						loc += result[ 1 ];
						handleElement( buffer, header.elements[ currentElement ].name, result[ 0 ] );
